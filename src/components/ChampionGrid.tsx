...
export default function ChampionGrid({
  ...
  return (
    <div className="flex flex-col">
      ...
      {/* Champion Grid */}
      <div className="bg-black/20 rounded-lg border border-gray-800 mb-3">
        <div className="h-[320px] overflow-y-auto hide-scrollbar">
            ...
            {filteredChampions.map((champion) => {
              ...
              return (
                <button
                  ...
                  disabled={isMyTurn && (isSelected || isBanned)}
                  className={`
                    w-[108px] h-[108px] rounded-lg overflow-hidden border-2 transition-all
                    ${isSelected ? 'border-yellow-500 opacity-50' : ''}
                    ${isBanned ? 'border-red-500 opacity-50' : ''}
                    ${isPending ? (
                      isPickPhase ? 'border-green-500 scale-105' : 'border-red-500 scale-105'
                    ) : 'border-transparent'}
                    ${isMyTurn && (isSelected || isBanned) ? 'cursor-not-allowed' : ''}
                    ${isMyTurn && !isSelected && !isBanned ? 'hover:scale-105 cursor-pointer' : ''}
                  `}
                >
                  <img
                    src={champion.image}
                    alt={champion.name}
                    className={`w-full h-full object-cover ${isSelected || isBanned ? 'grayscale' : ''}`}
                  />
                </button>
              );
            })}
